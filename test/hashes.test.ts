...
describe('hashes', () => {
  it('ed.hash() rejects a non-Uint8Array message before calling the configured SHA-512 provider', () => {
  ...
  it('ed.hash() rejects configured SHA-512 providers that return the wrong type or a digest not exactly 64 bytes', () => {
    const prev = ed.hashes.sha512;
    try {
  ...
  it('getPublicKey()/getPublicKeyAsync() reject configured providers that return digests not exactly 64 bytes', async () => {
  ...
  it('sign/signAsync/verify/verifyAsync reject configured SHA-512 providers that return digests not exactly 64 bytes', async () => {
