...
describe('ed25519', () => {
  ...
  it('should not accept >32byte private keys', async () => {
    ...
    // @ts-ignore
    expect(() => ed.getPublicKeyAsync(invalidPriv)).rejects.toThrowError();
  });
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (message: string, privateKey: bigint) => {
          const publicKey = await ed.getPublicKeyAsync(to64Bytes(privateKey));
          const signature = await ed.signAsync(to64Bytes(message), to64Bytes(privateKey));
          expect(publicKey.length).toBe(32);
          expect(signature.length).toBe(64);
          expect(await ed.verifyAsync(signature, to64Bytes(message), publicKey)).toBe(true);
        }
      ),
      { numRuns: 5 }
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (bytes, wrongBytes, privateKey) => {
          ed.utils.hexToBytes(privateKey.toString(16).padStart(64, '0'))
          const message = new Uint8Array(bytes);
          const wrongMessage = new Uint8Array(wrongBytes);
          const publicKey = await ed.getPublicKeyAsync(privateKey.toString(16).padStart(64, '0'));
          const signature = await ed.signAsync(message, to64Bytes(privateKey));
          expect(await ed.verifyAsync(signature, wrongMessage, publicKey)).toBe(
  ...
  describe('basic methods', () => {
    it('should sign and verify', async () => {
      ...
      expect(await ed.verifyAsync(signature, msg, publicKey)).toBe(true);
    });
    it('should not verify signature with wrong public key', async () => {
      ...
      const signature = await ed.signAsync(msg, privKey);
      expect(await ed.verifyAsync(signature, msg, publicKey)).toBe(false);
    });
    it('should not verify signature with wrong hash', async () => {
      const publicKey = await ed.getPublicKeyAsync(privKey);
      const signature = await ed.signAsync(msg, privKey);
      expect(await ed.verifyAsync(signature, wrongMsg, publicKey)).toBe(false);
    });
  });
  describe('sync methods', () => {
    it('should throw when sha512 is not set', () => {
      expect(() => ed.getPublicKey(privKey)).toThrowError();
      expect(() => ed.sign(msg, privKey)).toThrowError();
    });
    it('should sign and verify', () => {
      ed.utils.sha512Sync = (...m) => sha512(ed.utils.concatBytes(...m));
      const publicKey = ed.getPublicKey(privKey);
      const signature = ed.sign(msg, privKey);
      expect(ed.verify(signature, msg, publicKey)).toBe(true);
    });
    it('should not verify signature with wrong public key', () => {
      const publicKey = ed.getPublicKey(ed.utils.randomPrivateKey());
      const signature = ed.sign(msg, privKey);
      expect(ed.verify(signature, msg, publicKey)).toBe(false);
    });
    it('should not verify signature with wrong hash', () => {
      const publicKey = ed.getPublicKey(privKey);
      const signature = ed.sign(msg, privKey);
      expect(ed.verify(signature, wrongMsg, publicKey)).toBe(false);
    });
  });
  describe('BASE_POINT.multiply()', () => {
    ...
    it('should create right publicKey without SHA-512 hashing TEST 1', () => {
      const publicKey =
        G.multiply(0x90af56259a4b6bfbc4337980d5d75fbe3c074630368ff3804d33028e5dbfa77n);
    ...
    it('should create right publicKey without SHA-512 hashing TEST 2', () => {
      const publicKey =
        G.multiply(0x364e8711a60780382a5d57b061c126f039940f28a9e91fe039d4d3094d8b88n);
      expect(publicKey.toHex()).toBe(
    ...
    it('should create right publicKey without SHA-512 hashing TEST 3', () => {
      const publicKey =
        G.multiply(0xb9bf90ff3abec042752cac3a07a62f0c16cfb9d32a3fc2305d676ec2d86e941n);
      expect(publicKey.toHex()).toBe(
    ...
    it('should create right publicKey without SHA-512 hashing TEST 4', () => {
      const publicKey =
        G.multiply(0x69d896f02d79524c9878e080308180e2859d07f9f54454e0800e8db0847a46en);
      expect(publicKey.toHex()).toBe(
    ...
    it('should throw Point#multiply on TEST 5', () => {
      for (const num of [0n, 0, -1n, -1, 1.1]) {
        // @ts-ignore
        expect(() => ed.ExtendedPoint.BASE.multiply(num)).toThrowError();
      }
    });
  });
  ...
  //       const apub = await ed.getPublicKeyAsync(asec);
  //       const bsec = ed.utils.randomPrivateKey();
  //       const bpub = await ed.getPublicKeyAsync(bsec);
  //       try {
...
describe('ed25519 official vectors', () => {
  ...
  it('should match 1024 official vectors', async () => {
    for (let i = 0; i < vectors.length; i++) {
      ...
      // Calculate
      const pub = await ed.getPublicKeyAsync(to64Bytes(priv));
      expect(hex(pub)).toEqual(expectedPub);
      expect(pub).toEqual(ed.ExtendedPoint.fromHex(pub).toRawBytes());

      const signature = hex(await ed.signAsync(msg, priv));
...
describe('input immutability', () => {
  it('sign/verify are immutable', async () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      let signature = await ed.signAsync(payload, privateKey);
      if (!(await ed.verifyAsync(signature, payload, publicKey))) {
      ...
      if (!(await ed.verifyAsync(signatureCopy, payload, publicKey))) {
...
describe('ZIP-215 compliance tests', () => {
  ...
  it('should pass all of them', async () => {
    ...
    for (let v of zip215) {
      let noble = false;
      try {
        noble = await ed.verifyAsync(v.sig_bytes, str, v.vk_bytes);
      } catch (e) {
        noble = false;
      }
